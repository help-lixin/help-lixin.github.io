...
### (1). 我为什么学习HBase(以及看源码)?
> 我们知道,数据存储(INSERT/UPDATE/DELETE)比较简单,而基于数据检索(Query)反而是比较复杂的,特别是Sass化的今天,数据量庞大,检索的条件比较复杂(模拟检索).  
> 而在Query时,仍然,坚持使用RDBMS,会感觉到力不从心了(特别是模似检索),那么,业界有没有一个好的解决方案呢?  
> 在业界确实存在一套整合方案(HBase+Solr),大体的解决方案如下:   
> 1. 订阅MySQL binlog.  
...
> 8. 方案是比较完美,但是,说实话,开发人员需要懂:ES和Hbase,所以,我的想法是自己写ES Plugin,实现第6步和第7步的整个过程.  
> 9. ES的x-pack已经支持了SQL,为什么要还要自己写?因为x-pack是收费的(很贵),同时,我不仅要通过ES查询,还会要检索HBase,再进行组合.这个步骤,我也想通过SQL来做到,整个开发过程让开发是无感知的.    
...
> HBASE存储模型   
> cf(Column Family)简称:列簇,在Hbase里,列簇一般用于将相关的列(Cloumn)组合起来,在物理上Hbase其实是按照CF存储的.
> 在HBase里,要定位一个单元格数据必须要是如下格式:rowkwy+cf+time.
...
|  ----                  | ----                                                       |
|  cell                  |   数据                                                      |
|  column family         |   HBase会相关的列组合起来,并尽量放在同一台机器上,可以包含N个列      |
|  column                |   列是数据存储的最小单元                                       |
